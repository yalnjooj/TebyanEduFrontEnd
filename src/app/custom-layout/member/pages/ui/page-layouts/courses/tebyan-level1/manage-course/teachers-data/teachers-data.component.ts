...
import { Observable } from 'rxjs';
  ...
  @Output() senDataToManage: EventEmitter<any> = new EventEmitter<any>();
  ...
  constructor(
    ...
    private eventsSubscriptionAllTabs: Subscription;
    ...
    ngOnDestroy(): void {
      ...
      this.eventsSubscriptionAllTabs?.unsubscribe();
    ...
    ngOnInit(): void {
      ...
      this.unsubscription0 = this.apollo.watchQuery({
        ...
        this.senDataToManage.emit({activitiesBasicData: a, participationsBasicData: p, attendanceBasicData: at, shortTestsBasicData: {shortTestsBasicData: s, levelNumber: this.levelNumber}, oralExamBasicData: {oralExamBasicData: o, levelNumber: this.levelNumber}, writtenExamBasicData: w, courseNo: this.courseNo})

        this.dataSource = new MatTableDataSource(e);
        this.dataSource.sort = this.sort;
        this.dataSource.paginator = this.paginator;
  
        this.eventsSubscriptionAc = this.event.subscribe(() => {
          this.senDataToManage.emit({activitiesBasicData: a, participationsBasicData: p, attendanceBasicData: at, shortTestsBasicData: s, oralExamBasicData: o, writtenExamBasicData: w})
        });
  ...
  addNewStudent(){
      ...
      this.afterClosed = this.dialog.open(AddNewStudentComponent,{
          disableClose: true,
          width: '100vw',
          maxWidth: '100vw',
  ...
  deleteCourse(courseDetailsId, studentId){
    ...
    this.dialog.open(ConformDialogComponent,{
      ...
      if(!JSON.parse(result)) return
          ...
          this.unsubscription1 = this.apollo.mutate({
            mutation: gql`
              mutation deleteP01_personal($id: Int! $studentId: Int! $cource_id: Int!){
                deleteP01_personal(id: $id studentId: $studentId cource_id: $cource_id)
              }
              `,
              variables:{
                id: parseInt(courseDetailsId),
                studentId: parseInt(studentId),
                cource_id: parseInt(this.courseNo)
              }
          }).subscribe(( {data}: any ) => {
            this.ngOnInit()
...
ngOnInit(): void {
  ...
  this.form = this.fb.group({      
    ...
    neighborhood:  new FormControl({ value: null, disabled: false })
      ...
      this.unsubscription1 = this.apollo.watchQuery({
          query: gql`
            ...
            variables:{
              ...
              this.status = {status: 'edit', message: 'الرجاء تحديث البيانات'}
             ...
             await this.nationalitiesFunc(data.searchStudentId.basec?.sex_id)
             await this.residenceCountryId(data.searchStudentId.countryOfPersonal?.country_of_residence_id)

             await this.citiesFunc(data.searchStudentId.citieANDregion?.regionId)
             await this.neighborhoodFunc(data.searchStudentId.citieANDregion?.cityId)

              this.st = data.searchStudentId.countryOfPersonal?.country_of_residence_id
              ...
              this.snackBar.open('الرجاء تحديث البيانات','إغلاق', {
                duration: 6000,
                horizontalPosition: this.horizontalPosition,
                verticalPosition: this.verticalPosition,
              });
...
updateDate(){
  ...
  this.unsubscription6 = this.apollo.mutate({
    mutation: gql`
      mutation createP01_personal($identification: ID! $status: String! $st: Int $courseId: Int $nameAr: String $nameEn: String $sex: Int $catagoryId: Int $phone: String $companyPhoneKey: Int $email: String $birthDay: Date $nationalityId: Int $residenceId: Int $regionId: Int $cityId: Int $neighborhoodId: Int $qualificationId: Int $specialization: String $memorizing: Int $tajwed: Boolean $expertise: Int){
        createP01_personal(identification: $identification status: $status st: $st courseId: $courseId nameAr: $nameAr nameEn: $nameEn sex: $sex catagoryId: $catagoryId phone: $phone companyPhoneKey: $companyPhoneKey email: $email birthDay: $birthDay nationalityId: $nationalityId residenceId: $residenceId regionId: $regionId cityId: $cityId neighborhoodId: $neighborhoodId qualificationId: $qualificationId specialization: $specialization memorizing: $memorizing tajwed: $tajwed expertise: $expertise)
      }
      `,
      variables:{
