...
export class AttendanceComponent implements OnInit {
  ...
  @Input() set dataFromAttendances(localValue: any){
    ...
    if(localValue?.attendance){

      this.dataFromAttendance(localValue.attendance, localValue.attendanceBasicData)
  ...
  dataFromAttendance(localValue, namesInfoo){
      ...
      for (let index = 0; index < namesInfoo.length; index++) {
        ...
        namesInfoo[index]['درجة الحضور'] = sum.toFixed(1)
  ...
  constructor(
    ...
    op(val, column, studentId, status){
      ...
      let degd = (15 / this.data.courcesDates.length)
      ...
      if (status == undefined) {
        ...
        this.apollo.mutate({
          mutation: gql`
            ...
            variables:{
              ...
              studentId,
              column,
              value: (val.checked? degd : 0).toString(),          
            }
        }).subscribe(( {data}: any ) => {
          
          this.dataFromAttendance(data.createCourcesDatesValues, this.localValue.attendanceBasicData)
        ...
        this.apollo.mutate({
          ...
          this.dataFromAttendance(data.updateCourcesDatesValues, this.localValue.attendanceBasicData)
